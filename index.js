...
//   console.log("choosen section name", selectedSectionName);
//   const points2D = getPoints2DBySectionName(selectedSectionName);
//   console.log(`Selected section: ${selectedSectionName}`);
//   console.log("Points2D:", points2D);
//   draw2DShape(points2D);
// });

// function getPoints2DBySectionName(sectionName) {
//   const section = verticesData.polygonsBySection.find(
//     (sec) => sec.sectionName === sectionName
//   );
//   console.log("section here is", section);
//   if (section) {
//     console.log("skibidi", section.polygons);
//     return section.polygons.flatMap((polygon) => polygon.points2D);
//   }
//   return [];
// }

// function draw2DShape(points2D) {
//   d3.select("#svgContainer").selectAll("*").remove(); // Clear previous SVG elements
...
//   console.log("Drawing points2D:", points2D);

//   // Ensure points are within the SVG viewport
//   const margin = 20;
...
//   const xScale = d3
//     .scaleLinear()
//     .domain(d3.extent(points2D, (d) => d.vertex[0]))
//     .range([margin, width + margin]);

//   const yScale = d3
//     .scaleLinear()
//     .domain(d3.extent(points2D, (d) => d.vertex[1]))
...
//     .attr(
//       "d",
//       lineGenerator.x((d) => xScale(d.vertex[0])).y((d) => yScale(d.vertex[1]))
//     )
//     .attr("fill", "none")
//     .attr("stroke", "black");
// }
...
//     polygon.points3D.forEach((point, index) => {
//       if (index === 0) {
//         shape.moveTo(point.vertex[0], point.vertex[1]);
//       } else {
//         shape.lineTo(point.vertex[0], point.vertex[1]);
//       }
//     });
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  dropdown.addEventListener("change", (event) => {
    const selectedSectionName =
      event.target.options[event.target.selectedIndex].text;
    // draw3DShape(selectedSectionName);
    const points2D = getPoints2DBySectionName(selectedSectionName);
    draw2DShape(points2D);
  });
  ...
  //       section.polygons.forEach((polygon) => {
  //         polygon.points3D.forEach((point) => {
  //           allPoints.push(new THREE.Vector3(...point.vertex));
  //         });
  //       });
  //     });

  //     if (allPoints.length > 0) {
  //       // Create a geometry from collected points
  //       const geometry = new THREE.BufferGeometry();
  //       const vertices = new Float32Array(
  //         allPoints.flatMap((v) => [v.x, v.y, v.z])
  //       );
  //       geometry.setAttribute("position", new THREE.BufferAttribute(vertices, 3));

  //       // Create a material
  //       const material = new THREE.PointsMaterial({
  //         color: 0xffd700, // Example color (gold)
  //         size: 5, // Adjust point size
  //       });
  ...
  // Call the function on page load to draw the shape using all points
  //   window.onload = () => draw3DShape();

  //   function draw3DShape(selectedSectionName) {
  //     // Clear previous objects
  //     while (scene.children.length > 0) {
  //       scene.remove(scene.children[0]);
  //     }
  //     // Add light to the scene again
  //     scene.add(light);
  //     // Add grid helper to the scene
  //     scene.add(gridHelper);

  //     verticesData.polygonsBySection.forEach((section) => {
  //       if (section.sectionName === selectedSectionName) {
  //         section.polygons.forEach((polygon) => {
  //           const shape = new THREE.Shape();
  //           polygon.points3D.forEach((point, index) => {
  //             if (index === 0) {
  ...
  function draw2DShape(points2DBySection) {
    ...
    //   g.selectAll(".polygon").attr("transform", transform);
    // }
    // function zoomed(event) {
    //   const transform = event.transform;
